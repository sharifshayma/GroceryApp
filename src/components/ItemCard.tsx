"use client";

import { useT } from "@/i18n/LocaleProvider";
import { getItemName } from "@/lib/i18n-names";
import { IconCheckCircle, IconEdit, IconTrash } from "@/components/Icons";
import { ItemImage } from "@/components/ItemImage";

type ItemTag = {
  notes: string | null;
  tag: {
    id: string;
    name: string;
    color: string;
    type: "recipe" | "store" | "custom";
  };
};

type Item = {
  id: string;
  name: string;
  nameHe?: string | null;
  emoji?: string | null;
  defaultUnit: string;
  photoUrl?: string | null;
  tags: ItemTag[];
};

export function ItemCard({
  item,
  onEdit,
  onDelete,
  onAddToList,
  isInList = false,
  showActions = false,
  selectMode = false,
  isSelected = false,
  onSelect,
}: {
  item: Item;
  onEdit?: () => void;
  onDelete?: () => void;
  onAddToList?: (item: Item) => void;
  isInList?: boolean;
  showActions?: boolean;
  selectMode?: boolean;
  isSelected?: boolean;
  onSelect?: (item: Item) => void;
}) {
  const { t, locale } = useT();
  const itemTags = item.tags ?? [];

  const handleClick = () => {
    if (selectMode && onSelect) {
      onSelect(item);
    } else if (onAddToList) {
      // Open the modal regardless of list membership — the modal handles
      // showing current lists, stock, and pricing for the item.
      onAddToList(item);
    }
  };

  const cardIsClickable =
    (selectMode && !!onSelect) || (!selectMode && !!onAddToList);

  const unitLabel = t(`units.${item.defaultUnit}`);
  const unit = unitLabel.startsWith("units.") ? item.defaultUnit : unitLabel;

  return (
    <div
      onClick={cardIsClickable ? handleClick : undefined}
      className={`bg-white rounded-xl p-4 border shadow-sm transition-colors ${
        selectMode
          ? isSelected
            ? "border-primary bg-primary/5 cursor-pointer"
            : "border-neutral/20 cursor-pointer hover:border-primary/30"
          : cardIsClickable
            ? "border-neutral/20 hover:border-primary/30 cursor-pointer"
            : "border-neutral/20"
      }`}
    >
      <div className="flex items-center gap-3">
        {selectMode && (
          <span className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 ${
            isSelected ? "bg-primary border-primary text-white" : "border-neutral"
          }`}>
            {isSelected && <span className="text-xs">✓</span>}
          </span>
        )}
        <ItemImage item={item} size="md" />
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-text truncate">{getItemName(item, locale)}</p>
          <p className="text-xs text-text-secondary mt-0.5">
            {unit}
          </p>
        </div>
        {!selectMode && (
          <div className="flex items-center gap-0.5 flex-shrink-0">
            {onAddToList && isInList && (
              <span className="w-10 h-10 rounded-lg text-green flex items-center justify-center">
                <IconCheckCircle />
              </span>
            )}
            {showActions && (
              <>
                <button
                  onClick={(e) => { e.stopPropagation(); onEdit?.(); }}
                  className="w-10 h-10 rounded-lg text-text-secondary hover:text-primary hover:bg-primary/10 flex items-center justify-center transition-colors"
                >
                  <IconEdit />
                </button>
                <button
                  onClick={(e) => { e.stopPropagation(); onDelete?.(); }}
                  className="w-10 h-10 rounded-lg text-text-secondary hover:text-danger hover:bg-danger/10 flex items-center justify-center transition-colors"
                >
                  <IconTrash />
                </button>
              </>
            )}
          </div>
        )}
      </div>

      {/* Tag pills */}
      {itemTags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2 ps-9">
          {itemTags.slice(0, 3).map((it) => (
            <span
              key={it.tag.id}
              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium text-white"
              style={{ backgroundColor: it.tag?.color || "#3B82F6" }}
            >
              {it.tag?.type === "recipe" ? "🍽️" : it.tag?.type === "store" ? "🏪" : "🏷️"}
              {it.tag?.name}
            </span>
          ))}
          {itemTags.length > 3 && (
            <span className="text-[10px] text-text-secondary font-medium px-1">
              +{itemTags.length - 3}
            </span>
          )}
        </div>
      )}
    </div>
  );
}
